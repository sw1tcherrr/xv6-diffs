...
static struct spinlock wakeup_lock;
static int cnt;
static pagetable_t kernel_pagetable;


#define ITER_PROC(p) for ((p) = (struct proc*)proc_list.next; \
			  (struct list*)(p) != &proc_list;                   \
			  (p) = (struct proc*)(((struct list*)(p))->next))
...
void procinit(void) {
	...
	initlock(&wakeup_lock, "wakeup_lock");
	lst_init(&proc_list);
	cnt = -1;
}
...
static struct proc *allocproc(void) {
	...
	recursive_acquire(&list_lock);
	recursive_acquire(&wakeup_lock);
	cnt += 1;
	lst_push(&proc_list, &p->iter);
	recursive_release(&wakeup_lock);
	recursive_release(&list_lock);
...
void sched(void) {
	...
	if (!holding(&p->lock))
		panic("sched p->lock");
	if (mycpu()->noff != 1 + list_lock.depth)
		panic("sched locks");
	if (p->state == RUNNING)
		panic("sched running");
	if (intr_get())
